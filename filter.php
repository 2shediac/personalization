...
/**
 * Filter class for filter_personalization.
 *
 ...
 * Filter class
 */
class filter_personalization extends \moodle_text_filter {
                ...
                switch ($token[2]) {
                    ...
                    case 'GRADE':
                        $sql = 'SELECT *
                                 ...
                                 JOIN {grade_items} gi ON gi.id = gg.itemid AND gi.itemtype = ?
                                WHERE gi.courseid = ? AND gg.userid = ?';
                        $params = ['course', $course->id, $user->id];
                        $graderec = $DB->get_record_sql($sql, $params);

                        if (!empty($graderec)) {
                            if ($graderec->finalgrade == null) {
                                return get_string('nograde', 'filter_personalization');
                            } else {
                                return $graderec->finalgrade;
                            }
                        } else {
                            return get_string('nograde', 'filter_personalization');
                        }
            ...
            case 'TEACHER':
                ...
                switch ($token[2]) {
                    ...
                    case 'PICTURE':
                        $pics = [];
                        foreach ($users as $user) {
                            // TBD: possibly reduce picture size if count > X ?
                            $pics[] = $OUTPUT->user_picture($user, ['size' => 100, 'class' => 'userpicture']);
                        }
                        return implode('&#8202;', $pics); // TBD: pic separator?
